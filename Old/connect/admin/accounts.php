...
$stmt = $con->prepare('SELECT id, username, email, activation_code, role, registered, last_seen, approved FROM accounts ' . $where . ' ORDER BY ' . $order_by . ' ' . $order . ' LIMIT ?,?');
$types = '';
$params = [];
if ($search) {
    $params[] = &$param3;
...
$stmt->execute();
$stmt->bind_result($result_id, $result_username, $result_email, $result_activation_code, $result_role, $result_registered, $result_last_seen, $result_approved);
// Iterate the results
while($stmt->fetch()) {
    // Add result to accounts array
    $accounts[] = ['id' => $result_id, 'username' => $result_username, 'email' => $result_email, 'activation_code' => $result_activation_code, 'role' => $result_role, 'registered' => $result_registered, 'last_seen' => $result_last_seen, 'approved' => $result_approved];
}
// Delete account
...
if (isset($_GET['deactivate'])) {
    // Update the account
    $stmt = $con->prepare('UPDATE accounts SET activation_code = "deactivated", remember_me_code = "" WHERE id = ?');
    $stmt->bind_param('i', $_GET['deactivate']);
    $stmt->execute();
...
if (isset($_GET['activate'])) {
    // Update the account
    $stmt = $con->prepare('UPDATE accounts SET activation_code = "activated" WHERE id = ?');
...
if (isset($_GET['approve'])) {
    // Update the account
    $stmt = $con->prepare('UPDATE accounts SET approved = 1 WHERE id = ?');
