...
$accounts = $con->query('SELECT * FROM accounts WHERE cast(registered as DATE) = cast("' . $date . '" as DATE) ORDER BY registered DESC')->fetch_all(MYSQLI_ASSOC);
// Total accounts that were last active over a month ago
$inactive_accounts = $con->query('SELECT COUNT(*) AS total FROM accounts WHERE last_seen < date_sub("' . $date . '", interval 1 month)')->fetch_object()->total;
// Accounts that are active in the last day
$active_accounts = $con->query('SELECT * FROM accounts WHERE last_seen > date_sub("' . $date . '", interval 1 day) ORDER BY last_seen DESC')->fetch_all(MYSQLI_ASSOC);
// Total accounts that are active in the last month
$active_accounts_total = $con->query('SELECT COUNT(*) AS total FROM accounts WHERE last_seen > date_sub("' . $date . '", interval 1 month)')->fetch_object()->total;
?>
